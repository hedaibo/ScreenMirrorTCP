...
//    private DataOutputStream mDataOutputStream;
	...
	public void start(ArrayList<SocketMole> listSocketMoles, MediaProjection mediaProjection,IDisconnectCallback disconnectCallback) {
...
//		Log.v(TAG, "onImageData  " + buf.length + "  ------  mListSocketMoles.size():"+mListSocketMoles.size());
		if (mListSocketMoles.size() > 0) {
			SocketMole socketMole = null;
			try {
				...
				for (int i = 0; i < mListSocketMoles.size(); i++) {
					socketMole = mListSocketMoles.get(i);
					if (socketMole != null && socketMole.isConnect()) {
						DataOutputStream videoDos = socketMole.getVideoDos();
				...
				if (mDisconnectCallback != null) {
					mDisconnectCallback.onDisconnect(socketMole);
				}
